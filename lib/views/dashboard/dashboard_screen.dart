...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Widget build(BuildContext context) {
    final pages = [
      DashboardContent(
        onNavigate: (int index) => setState(() => _currentIndex = index),
      ),
        ...
        if (mounted) {
          setState(() {
            ...
            bool sameDay(DateTime d) =>
                ...
                d.month == today.month &&
                d.day == today.day;
              final todayRecords = parsed.where(
                (m) =>
                    (m['_ts'] as DateTime?) != null &&
                    sameDay(m['_ts'] as DateTime),
              ).toList();
            ...
            for (final r in todayRecords) {
              final typeStr = (r['type'] ?? r['action'] ?? '')
                  .toString()
                  .toLowerCase();
              final ts = r['_ts'] as DateTime?;
              if (ts == null) continue;
              if (entradaDt == null && (typeStr.contains('entrada') || typeStr.contains('entry'))) {
                entradaDt = ts;
              }
              if (typeStr.contains('saida') ||
                  typeStr.contains('saída') ||
                  typeStr.contains('exit')) {
            ...
            if (todayRecords.isNotEmpty) {
              ...
              entrySubtitle = last['location'] is Map
                  ? (last['location']['name'] ?? entrySubtitle)
                  : (last['location']?.toString() ?? entrySubtitle);
            }
            ...
            for (final entry in byDay.entries) {
              ...
              for (final ev in events) {
                ...
                if (typeStr.contains('entrada') ||
                    typeStr.contains('entry') ||
                    typeStr.contains('in')) {
                  lastEntry = ts;
                } else if (typeStr.contains('saida') ||
                    typeStr.contains('saída') ||
                    typeStr.contains('exit') ||
            ...
            // ignore: avoid_print
            print(
              '[DASHBOARD DEBUG] totalWeek=${totalWeek.inMinutes}min presentDays=${presentDays.length}',
            );
            if (diff.isNegative) {
              overtime = '0h 00m';
              balance = fmtDur(diff);
            ...
            recentRecords = parsed.take(5).map<Map<String, dynamic>>((r) {
              ...
              if (r['entryTime'] != null) {
                try {
                  final dte = DateTime.parse(
                    r['entryTime'].toString(),
                  ).toLocal();
          ...
          child: SizedBox(
            ...
            child: Stack(
              children: [
                Positioned.fill(
                  child: CustomPaint(
                    painter: _HeaderBackgroundPainter(theme: theme),
                  ),
                ),
                Positioned.fill(
                  child: Container(
                            ...
                            try {
                              ...
                              final same = recentRecords.where((rr) {
                                final t = (rr['type'] ?? '')
                                    .toString()
                                    .toLowerCase();
                                return t.contains(lowerType) ||
                                    lowerType.contains(t);
                              }).toList();
                              occurrence =
                                  same.indexWhere((map) => map == r) + 1;
